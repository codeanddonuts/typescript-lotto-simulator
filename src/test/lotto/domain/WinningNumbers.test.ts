...
describe("Are valid winning numbers?", () => {
  ...
  it ("No: Not enough prizes", () => {
    expect(() => new WinningNumbers(new Round(123), new Game([1, 2, 3, 4, 5, 6]), 7, [])).toThrow()
  })

  it ("No: Too many prizes", () => {
    expect(() => new WinningNumbers(new Round(123), new Game([1, 2, 3, 4, 5, 6]), 7, [10, 9, 8, 7, 6, 5])).toThrow()
  })
})
...
describe("Prize of ...", () => {
  it("Jackpot = 1,000,000,000", () => expect(winningNumbers.prizeOf(Tiers.JACKPOT)).toEqual(1_000_000_000))

  it("2nd place = 50,000,000", () => expect(winningNumbers.prizeOf(Tiers.SECOND)).toEqual(50_000_000))

  it("3rd place = 1,500,000", () => expect(winningNumbers.prizeOf(Tiers.THIRD)).toEqual(1_500_000))

  it("4th place = 50,000", () => expect(winningNumbers.prizeOf(Tiers.FOURTH)).toEqual(50_000))

  it("5th place = 5,000", () => expect(winningNumbers.prizeOf(Tiers.FIFTH)).toEqual(5_000))
  
  it("5th place = 5,000", () => expect(winningNumbers.prizeOf(Tiers.NONE)).toEqual(0))
})
