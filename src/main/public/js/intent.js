...
export const intent = () => {
  return {
    ...
    addManualPick$: rxjs.fromEvent(document, "click").pipe(filter(event => event.target.id === "add-manual-picks")),
    removeManualPick$: rxjs.fromEvent(document, "click").pipe(filter(event => event.target.id === "remove-manual-picks")),
    submitPurchase$: rxjs.fromEvent(document, "click").pipe(filter(event => event.target.id === "purchase")),
